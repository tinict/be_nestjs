...
export class CampaignService {
    constructor(
        ...
        private categoryRepository: Repository<CampaignEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: CampaignCreateModel, credentials: any) => {
    ...
    all = async (query: CampaignQueryModel, credentials: any) => {
        ...
        QueryHelper.assignQuery(
            where,
            CampaignFields.allowQueryField,
            query,
        );
        ...
        const [rows, count] = await this.categoryRepository.findAndCount({
            ...
            relations: QueryHelper.GetRelations(
                _.get(query, 'include', ''),
                CampaignFields.allowRelationKey,
            ),
            order: {
                DisplayOrder: 'ASC',
            },
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(CampaignEntity.name),
            );
        }
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(CampaignEntity.name),
            );
        }
    ...
    bulk = async (models: CampaignCreateModel[], credentials: any) => {
        await Promise.all(
