...
export class ContactService {
    constructor(
        ...
        private categoryRepository: Repository<ContactEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: ContactCreateModel, credentials: any) => {
    ...
    all = async (query: ContactQueryModel, credentials: any) => {
        ...
        const [rows, count] = await this.categoryRepository.findAndCount({
            ...
            relations: QueryHelper.GetRelations(
                _.get(query, 'include', ''),
                ContactFields.allowRelationKey,
            ),
            order: {
                DisplayOrder: 'ASC',
            },
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(ContactEntity.name),
            );
        }
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(ContactEntity.name),
            );
        }
    ...
    bulk = async (models: ContactCreateModel[], credentials: any) => {
        await Promise.all(
