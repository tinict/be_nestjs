...
export class CandidateService {
    constructor(
        ...
        private categoryRepository: Repository<CandidateEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: CandidateCreateModel, credentials: any) => {
    ...
    all = async (query: CandidateQueryModel, credentials: any) => {
        ...
        QueryHelper.assignQuery(
            where,
            CandidateFields.allowQueryField,
            query,
        );
        ...
        const [rows, count] = await this.categoryRepository.findAndCount({
            ...
            relations: QueryHelper.GetRelations(
                _.get(query, 'include', ''),
                CandidateFields.allowRelationKey,
            ),
            order: {
                DisplayOrder: 'ASC',
            },
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(CandidateEntity.name),
            );
        }
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(CandidateEntity.name),
            );
        }
    ...
    bulk = async (models: CandidateCreateModel[], credentials: any) => {
        await Promise.all(
