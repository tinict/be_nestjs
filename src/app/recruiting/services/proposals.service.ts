...
export class ProposalService {
    constructor(
        ...
        private categoryRepository: Repository<ProposalEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: ProposalCreateModel, credentials: any) => {
    ...
    all = async (query: ProposalQueryModel, credentials: any) => {
        ...
        QueryHelper.assignQuery(
            where,
            ProposalFields.allowQueryField,
            query,
        );
        ...
        const [rows, count] = await this.categoryRepository.findAndCount({
            ...
            relations: QueryHelper.GetRelations(
                _.get(query, 'include', ''),
                ProposalFields.allowRelationKey,
            ),
            order: {
                DisplayOrder: 'ASC',
            },
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(ProposalEntity.name),
            );
        }
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(ProposalEntity.name),
            );
        }
    ...
    bulk = async (models: ProposalCreateModel[], credentials: any) => {
        await Promise.all(
