...
export class CandidateQualificationService {
  constructor(
    @InjectRepository(CandidateQualificationEntity)
    private categoryRepository: Repository<CandidateQualificationEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CandidateQualificationCreateModel, credentials: any) => {
  ...
  all = async (query: CandidateQualificationQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CandidateQualificationFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CandidateQualificationFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CandidateQualificationEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CandidateQualificationUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CandidateQualificationEntity.name),
      );
    }
  ...
  bulk = async (models: CandidateQualificationCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CandidateQualificationEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
