...
@Injectable()
export class ContactSkillService {
    constructor(
        @InjectRepository(ContactSkillEntity)
        private categoryRepository: Repository<ContactSkillEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: ContactSkillCreateModel, credentials: any) => {
    ...
    all = async (query: ContactSkillQueryModel, credentials: any) => {
        ...
        QueryHelper.assignQuery(
            where,
            ContactSkillFields.allowQueryField,
            query,
        );
        ...
        const [rows, count] = await this.categoryRepository.findAndCount({
            ...
            relations: QueryHelper.GetRelations(
                _.get(query, 'include', ''),
                ContactSkillFields.allowRelationKey,
            ),
            order: {
                DisplayOrder: 'ASC',
            },
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(ContactSkillEntity.name),
            );
        }
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(ContactSkillEntity.name),
            );
        }
    ...
    bulk = async (models: ContactSkillCreateModel[], credentials: any) => {
        await Promise.all(
