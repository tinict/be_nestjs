...
export class ChannelService {
    constructor(
        ...
        private categoryRepository: Repository<ChannelEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: ChannelCreateModel, credentials: any) => {
    ...
    all = async (query: ChannelQueryModel, credentials: any) => {
        ...
        const [rows, count] = await this.categoryRepository.findAndCount({
            ...
            relations: QueryHelper.GetRelations(
                _.get(query, 'include', ''),
                ChannelFields.allowRelationKey,
            ),
            order: {
                DisplayOrder: 'ASC',
            },
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(ChannelEntity.name),
            );
        }
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(ChannelEntity.name),
            );
        }
    ...
    bulk = async (models: ChannelCreateModel[], credentials: any) => {
        await Promise.all(
