...
export class CandidateEducationService {
    constructor(
        ...
        private categoryRepository: Repository<CandidateEducationEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: CandidateEducationCreateModel, credentials: any) => {
    ...
    all = async (query: CandidateEducationQueryModel, credentials: any) => {
        ...
        QueryHelper.assignQuery(
            where,
            CandidateEducationFields.allowQueryField,
            query,
        );
        ...
        const [rows, count] = await this.categoryRepository.findAndCount({
            ...
            relations: QueryHelper.GetRelations(
                _.get(query, 'include', ''),
                CandidateEducationFields.allowRelationKey,
            ),
            order: {
                DisplayOrder: 'ASC',
            },
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(CandidateEducationEntity.name),
            );
        }
    ...
    update = async (
        id: string,
        body: CandidateEducationUpdateModel,
        credentials: any,
    ) => {
        await this.validateReference(body, credentials);
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(CandidateEducationEntity.name),
            );
        }
    ...
    bulk = async (models: CandidateEducationCreateModel[], credentials: any) => {
        await Promise.all(
            _.map(models, async (item) => {
                ...
                if (entity === null) {
                    entity = new CandidateEducationEntity();
                    entity.CreatedBy = _.get(credentials, 'UserId');
                    entity.OrganizationId = _.get(credentials, 'OrganizationId');
                }
