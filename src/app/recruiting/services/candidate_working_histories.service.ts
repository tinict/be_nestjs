...
@Injectable()
export class CandidateWorkingHistoryService {
  constructor(
    @InjectRepository(CandidateWorkingHistoryEntity)
    private categoryRepository: Repository<CandidateWorkingHistoryEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CandidateWorkingHistoryCreateModel, credentials: any) => {
  ...
  all = async (query: CandidateWorkingHistoryQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CandidateWorkingHistoryFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CandidateWorkingHistoryFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CandidateWorkingHistoryEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CandidateWorkingHistoryUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CandidateWorkingHistoryEntity.name),
      );
    }
  ...
  bulk = async (models: CandidateWorkingHistoryCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CandidateWorkingHistoryEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
