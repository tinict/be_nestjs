...
export class PositionService {
    constructor(
        ...
        private categoryRepository: Repository<PositionEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: PositionCreateModel, credentials: any) => {
    ...
    all = async (query: PositionQueryModel, credentials: any) => {
        ...
        QueryHelper.assignQuery(
            where,
            PositionFields.allowQueryField,
            query,
        );
        ...
        const [rows, count] = await this.categoryRepository.findAndCount({
            ...
            relations: QueryHelper.GetRelations(
                _.get(query, 'include', ''),
                PositionFields.allowRelationKey,
            ),
            order: {
                DisplayOrder: 'ASC',
            },
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(PositionEntity.name),
            );
        }
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(PositionEntity.name),
            );
        }
    ...
    bulk = async (models: PositionCreateModel[], credentials: any) => {
        await Promise.all(
