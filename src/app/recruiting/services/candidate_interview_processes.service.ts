...
@Injectable()
export class CandidateInterviewProcessService {
  constructor(
    @InjectRepository(CandidateInterviewProcessEntity)
    private categoryRepository: Repository<CandidateInterviewProcessEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CandidateInterviewProcessCreateModel, credentials: any) => {
  ...
  all = async (query: CandidateInterviewProcessQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CandidateInterviewProcessFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CandidateInterviewProcessFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CandidateInterviewProcessEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CandidateInterviewProcessUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CandidateInterviewProcessEntity.name),
      );
    }
  ...
  bulk = async (models: CandidateInterviewProcessCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CandidateInterviewProcessEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
