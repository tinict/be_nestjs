...
export class CandidateSkillService {
  constructor(
    ...
    private categoryRepository: Repository<CandidateSkillEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CandidateSkillCreateModel, credentials: any) => {
  ...
  all = async (query: CandidateSkillQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CandidateSkillFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CandidateSkillFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CandidateSkillEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CandidateSkillUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CandidateSkillEntity.name),
      );
    }
  ...
  bulk = async (models: CandidateSkillCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CandidateSkillEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
