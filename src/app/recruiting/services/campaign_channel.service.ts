...
@Injectable()
export class CampaignChannelService {
  constructor(
    @InjectRepository(CampaignChannelEntity)
    private categoryRepository: Repository<CampaignChannelEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CampaignChannelCreateModel, credentials: any) => {
  ...
  all = async (query: CampaignChannelQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CampaignChannelFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CampaignChannelFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CampaignChannelEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CampaignChannelUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CampaignChannelEntity.name),
      );
    }
  ...
  bulk = async (models: CampaignChannelCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CampaignChannelEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
