...
export class CampaignContactService {
    constructor(
        ...
        private categoryRepository: Repository<CampaignContactEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: CampaignContactCreateModel, credentials: any) => {
    ...
    all = async (query: CampaignContactQueryModel, credentials: any) => {
        ...
        QueryHelper.assignQuery(
            where,
            CampaignContactFields.allowQueryField,
            query,
        );
        ...
        const [rows, count] = await this.categoryRepository.findAndCount({
            ...
            relations: QueryHelper.GetRelations(
                _.get(query, 'include', ''),
                CampaignContactFields.allowRelationKey,
            ),
            order: {
                DisplayOrder: 'ASC',
            },
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(CampaignContactEntity.name),
            );
        }
    ...
    update = async (
        id: string,
        body: CampaignContactUpdateModel,
        credentials: any,
    ) => {
        await this.validateReference(body, credentials);
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(CampaignContactEntity.name),
            );
        }
    ...
    bulk = async (models: CampaignContactCreateModel[], credentials: any) => {
        await Promise.all(
            _.map(models, async (item) => {
                ...
                if (entity === null) {
                    entity = new CampaignContactEntity();
                    entity.CreatedBy = _.get(credentials, 'UserId');
                    entity.OrganizationId = _.get(credentials, 'OrganizationId');
                }
