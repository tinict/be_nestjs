...
export class CandidateLanguageService {
  constructor(
    @InjectRepository(CandidateLanguageEntity)
    private categoryRepository: Repository<CandidateLanguageEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CandidateLanguageCreateModel, credentials: any) => {
  ...
  all = async (query: CandidateLanguageQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CandidateLanguageFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CandidateLanguageFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CandidateLanguageEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CandidateLanguageUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CandidateLanguageEntity.name),
      );
    }
  ...
  bulk = async (models: CandidateLanguageCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CandidateLanguageEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
