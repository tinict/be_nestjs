...
@Injectable()
export class CampaignProposalPositionConditionService {
  constructor(
    @InjectRepository(CampaignProposalPositionConditionEntity)
    private categoryRepository: Repository<CampaignProposalPositionConditionEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CampaignProposalPositionConditionCreateModel, credentials: any) => {
  ...
  all = async (query: CampaignProposalPositionConditionQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CampaignProposalPositionConditionFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CampaignProposalPositionConditionFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CampaignProposalPositionConditionEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CampaignProposalPositionConditionUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CampaignProposalPositionConditionEntity.name),
      );
    }
  ...
  bulk = async (models: CampaignProposalPositionConditionCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CampaignProposalPositionConditionEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
