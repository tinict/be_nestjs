...
export class CampaignCandidateService {
  constructor(
    @InjectRepository(CampaignCandidateEntity)
    private categoryRepository: Repository<CampaignCandidateEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CampaignCandidateCreateModel, credentials: any) => {
  ...
  all = async (query: CampaignCandidateQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CampaignCandidateFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CampaignCandidateFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CampaignCandidateEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CampaignCandidateUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CampaignCandidateEntity.name),
      );
    }
  ...
  bulk = async (models: CampaignCandidateCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CampaignCandidateEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
