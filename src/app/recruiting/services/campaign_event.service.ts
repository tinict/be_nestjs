...
export class CampaignEventService {
  constructor(
    @InjectRepository(CampaignEventEntity)
    private categoryRepository: Repository<CampaignEventEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CampaignEventCreateModel, credentials: any) => {
  ...
  all = async (query: CampaignEventQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CampaignEventFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CampaignEventFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CampaignEventEntity.name),
      );
    }
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CampaignEventEntity.name),
      );
    }
  ...
  bulk = async (models: CampaignEventCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CampaignEventEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
