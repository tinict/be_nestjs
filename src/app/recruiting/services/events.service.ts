...
export class EventService {
    constructor(
        ...
        private categoryRepository: Repository<EventEntity>, // private organizationService: OrganizationService,
    ) { }
    ...
    create = async (body: EventCreateModel, credentials: any) => {
    ...
    all = async (query: EventQueryModel, credentials: any) => {
    ...
    findByIdOrFail = async (
        ...
        if (_.isNil(category)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_OBJ_NOT_FOUND(EventEntity.name),
            );
        }
    ...
    async findByIdsOrFail(ids: string[]) {
        ...
        if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
            throw ResponseHelper.NotFound(
                MSG.MSG_ARR_NOT_FOUND(EventEntity.name),
            );
        }
    ...
    bulk = async (models: EventCreateModel[], credentials: any) => {
        await Promise.all(
