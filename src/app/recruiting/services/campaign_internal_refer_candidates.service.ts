...
@Injectable()
export class CampaignInternalReferCandidateService {
  constructor(
    @InjectRepository(CampaignInternalReferCandidateEntity)
    private categoryRepository: Repository<CampaignInternalReferCandidateEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CampaignInternalReferCandidateCreateModel, credentials: any) => {
  ...
  all = async (query: CampaignInternalReferCandidateQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CampaignInternalReferCandidateFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CampaignInternalReferCandidateFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CampaignInternalReferCandidateEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CampaignInternalReferCandidateUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CampaignInternalReferCandidateEntity.name),
      );
    }
  ...
  bulk = async (models: CampaignInternalReferCandidateCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CampaignInternalReferCandidateEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
