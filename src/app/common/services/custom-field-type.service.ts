...
export class CustomFieldTypeService {
  constructor(
    @InjectRepository(CustomFieldTypeEntity)
    private categoryRepository: Repository<CustomFieldTypeEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CustomFieldTypeCreateModel, credentials: any) => {
  ...
  all = async (query: CustomFieldTypeQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CustomFieldTypeFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CustomFieldTypeFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CustomFieldTypeEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CustomFieldTypeUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CustomFieldTypeEntity.name),
      );
    }
  ...
  bulk = async (models: CustomFieldTypeCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CustomFieldTypeEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
