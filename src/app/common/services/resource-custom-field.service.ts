...
@Injectable()
export class ResourceCustomFieldService {
  constructor(
    @InjectRepository(ResourceCustomFieldEntity)
    private categoryRepository: Repository<ResourceCustomFieldEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: ResourceCustomFieldCreateModel, credentials: any) => {
  ...
  all = async (query: ResourceCustomFieldQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      ResourceCustomFieldFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        ResourceCustomFieldFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(ResourceCustomFieldEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: ResourceCustomFieldUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(ResourceCustomFieldEntity.name),
      );
    }
  ...
  bulk = async (models: ResourceCustomFieldCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new ResourceCustomFieldEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
