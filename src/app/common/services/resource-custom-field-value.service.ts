...
@Injectable()
export class ResourceCustomFieldValueService {
  constructor(
    @InjectRepository(ResourceCustomFieldValueEntity)
    private categoryRepository: Repository<ResourceCustomFieldValueEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (
    body: ResourceCustomFieldValueCreateModel,
    credentials: any,
  ) => {
    ...
    entity.ResourceId = body.resource_id;
    entity.CustomFieldId = body.custom_field_id;
    entity.CustomFieldValueId = body.custom_field_value_id;
  ...
  all = async (query: ResourceCustomFieldValueQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      ResourceCustomFieldValueFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        ResourceCustomFieldValueFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(ResourceCustomFieldValueEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: ResourceCustomFieldValueUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(ResourceCustomFieldValueEntity.name),
      );
    }
  ...
  bulk = async (
    models: ResourceCustomFieldValueCreateModel[],
    credentials: any,
  ) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new ResourceCustomFieldValueEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
