...
export class CustomFieldValueService {
  constructor(
    ...
    private categoryRepository: Repository<CustomFieldValueEntity>, // private organizationService: OrganizationService,
  ) {}
  ...
  create = async (body: CustomFieldValueCreateModel, credentials: any) => {
  ...
  all = async (query: CustomFieldValueQueryModel, credentials: any) => {
    ...
    QueryHelper.assignQuery(
      where,
      CustomFieldValueFields.allowQueryField,
      query,
    );
    ...
    const [rows, count] = await this.categoryRepository.findAndCount({
      ...
      relations: QueryHelper.GetRelations(
        _.get(query, 'include', ''),
        CustomFieldValueFields.allowRelationKey,
      ),
      order: {
        DisplayOrder: 'ASC',
      },
  ...
  findByIdOrFail = async (
    ...
    if (_.isNil(category)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_OBJ_NOT_FOUND(CustomFieldValueEntity.name),
      );
    }
  ...
  update = async (
    id: string,
    body: CustomFieldValueUpdateModel,
    credentials: any,
  ) => {
    await this.validateReference(body, credentials);
  ...
  async findByIdsOrFail(ids: string[]) {
    ...
    if (_.isEmpty(categorys) || _.size(categorys) !== _.size(ids)) {
      throw ResponseHelper.NotFound(
        MSG.MSG_ARR_NOT_FOUND(CustomFieldValueEntity.name),
      );
    }
  ...
  bulk = async (models: CustomFieldValueCreateModel[], credentials: any) => {
    await Promise.all(
      _.map(models, async (item) => {
        ...
        if (entity === null) {
          entity = new CustomFieldValueEntity();
          entity.CreatedBy = _.get(credentials, 'UserId');
          entity.OrganizationId = _.get(credentials, 'OrganizationId');
        }
